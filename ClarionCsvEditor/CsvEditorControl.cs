...
using static ClarionCsvEditor.JsonInterop;
...
namespace ClarionCsvEditor
{
    ...
    ///   JS -> C# (object) : { type: "ready" | "dirtyChanged" | "saveRequested" | "darkModeChanged" }
        ...
        private void HandleWebMessage(string message)
        {
            try
            {
                ...
                switch (type)
                {
                    ...
                    case "dirtyChanged":
                        var dirty = ExtractJsonValue(message, "dirty");
                        IsDirty = string.Equals(dirty, "true", StringComparison.OrdinalIgnoreCase);
                        break;
